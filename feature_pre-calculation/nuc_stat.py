...
class SlideProperty():
    ...
    def get_nucstat(self):
        ...
        nuc_stat = nuc_stat.iloc[:100, :]  # For testing, limit to first 100 nuclei
        ...
        nucstat_all = self.nuc_stat_processed.copy().values
        print(nucstat_all.shape)
        print(nucstat_all.dtype)
        print(nucstat_all[:5])
    ...
    def get_nucstat_parallel(self):
        ...
        # Now define process_single_nucleus
        def process_single_nucleus(id):
            try:
                ...
                magnification = 40
                resolution = np.max([1, np.round(1 / int(magnification) * stat['area']*0.002)])
                ...
                return list(stat_color.values()) + list(stat_cyto.values()) + list(stat_morphology.values()) + \
                    list(stat_haralick.values()) + list(df_gradient.values.reshape(-1)) + list(df_intensity.values.reshape(-1)) + list(df_fsd.values.reshape(-1))
            except Exception as e:
